...
// Helper function to check deep equality (simplified version)
const isDeepEqual = (a: any, b: any): boolean => {
  return JSON.stringify(a) === JSON.stringify(b);
};

// Helper function to get preset theme styles
const getPresetThemeStyles = (preset: string) => {
  // Import defaultPresets here to avoid circular dependency
  const { defaultPresets } = require("../utils/theme-presets");
  const presetData = defaultPresets[preset];
  if (presetData) {
    return {
      light: { ...presetData.styles.light },
      dark: { ...presetData.styles.dark },
    };
  }
